...
public class RobotContainer {
  ...
  //Command Groups
  //private final ParallelCommandGroup m_collect = new Collect();
...
//   private final Command driveXY = new DriveXY(x, y, angleDegrees, speed);
  ...
  private void configureButtonBindings() {
    ...
    new Trigger(
        ...
        .whenActive(new Collect());

    //Manual Shoot
    new Trigger(
            () -> {
              return shooterController.getRightTriggerAxis() > 0.1;    
            })
        .whenActive(new ManualShootCommand());
